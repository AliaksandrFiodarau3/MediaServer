...
public class AccessFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
      ...
      /*  String login = (String) req.getSession().getAttribute(Attribute.ATTRIBUTE_USER_LOGIN);
        ...
        String login = String.valueOf(CookieAction.getCookie(req, String.valueOf(Attribute.ATTRIBUTE_USER_LOGIN)));
        String root = String.valueOf(CookieAction.getCookie(req, String.valueOf(Attribute.ATTRIBUTE_USER_ROLE)));

        String command = req.getParameter(Parameter.PARMETER_COMMAND);

        System.out.println("Command: " + command + " Login: " + login + " Root: " + root);

        if (command != null) {

            if (command.equals("sign-out")) {
            ...
            if (login != null) {
                if (Boolean.valueOf(root) && commandSetForAdmin.contains(commandName)) {
                    System.out.println("1");
                    filterChain.doFilter(req, resp);
                    //resp.sendRedirect(Path.PATH_TO_ADMIN_PAGE);
                }
                if (!Boolean.valueOf(root) && commandSetForUser.contains(commandName)) {
                    System.out.println("2");
                    filterChain.doFilter(req, resp);
                    // resp.sendRedirect(Path.PATH_TO_USER_PAGE);
                } else {
                    System.out.println("3");
                    resp.sendRedirect(Path.PATH_TO_HOME_PAGE);
                }
            } else {

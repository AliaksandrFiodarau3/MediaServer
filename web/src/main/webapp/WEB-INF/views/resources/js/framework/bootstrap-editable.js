...
(function ($) {
    ...
    var Editable = function (element, options) {
        ...
        //apply options
        this.settings =
            $.extend({}, $.fn.editable.defaults, $.fn.editable.types.defaults, typeDefaults, options,
                     this.$element.data());
        ...
        $('body').on('click.editable', '.editable-popover', function (e) {
            e.stopPropagation();
        });
            ...
            switch (this.settings.autotext) {
                ...
                case 'auto':
                    if (this.$element.html().length) {
                        ...
                        //for SELECT do not use autotext when source is url and autotext = 'auto' (to prevent extra
                        // request)
                        if (type === 'select') {
    ...
    Editable.prototype = {
        ...
        save: function (value) {
            $.when(this.send(value))
                .done($.proxy(function (data) {
                    ...
                    if (isAjax && typeof this.settings.success === 'function' && (error =
                            this.settings.success.apply(this, arguments))) {
                ...
                .fail($.proxy(function (xhr) {
                    var msg = (typeof this.settings.error === 'function') ? this.settings.error.apply(this, arguments)
                        : null;
        ...
        send: function (value) {
            ...
            } else if (typeof this.settings.pk === 'string' && $(this.settings.pk).length === 1 && $(
            ...
            send =
                (this.settings.url !== undefined) && ((this.settings.send === 'always') || (this.settings.send
                                                                                            === 'auto' && pk)
                                                      || (this.settings.send === 'ifpk' /* deprecated */ && pk));
            ...
            if (send) { //send to server
                ...
                //creating params
                params =
                    (typeof this.settings.params === 'string') ? {params: this.settings.params} : $.extend({},
                ...
                return $.ajax({
                                  url: (typeof this.settings.url === 'function') ? this.settings.url.call(this)
                                      : this.settings.url,
                                  data: params,
                                  type: 'post',
                                  dataType: 'json'
        ...
        setPosition: function () {
            var p = this.$element.data('popover'), $tip = p.tip(), inside = false, placement, pos, actualWidth,
                actualHeight, tp;

            placement =
                typeof p.options.placement === 'function' ? p.options.placement.call(p, $tip[0], p.$element[0])
                    : p.options.placement;
            ...
            switch (inside ? placement.split(' ')[1] : placement) {
                ...
                case 'top':
                    ...
                    if ($tip.find('.arrow').get(0).offsetHeight === 10) {
                        actualHeight += 10;
                    }
                ...
                case 'left':
                    /* For Bootstrap 2.1.x: 10 pixels needed to correct popover position. See https://github.com/twitter/bootstrap/issues/4665 */
                    if ($tip.find('.arrow').get(0).offsetWidth === 10) {
                        actualWidth += 10;
                    }
        ...
        enableLoading: function () {
            ...
            var $tip = this.$element.data('popover').$tip;
            $tip.find('.editable-loading').css(
                {height: this.$content[0].offsetHeight, width: this.$content[0].offsetWidth});
    ...
    $.fn.editable.defaults = {
        ...
        send: 'auto', // strategy for sending data on server: 'always', 'never', 'auto' (default). 'auto' = 'ifpk'
                      // (deprecated)
        autotext: 'auto', //can be auto|never|always. Useful for select element: if 'auto' -> element text will be
                          // automatically set by provided value and source (in case source is object so no extra
                          // request will be performed).
        enablefocus: false, //wether to return focus on link after popover is closed. It's more functional, but focused
                            // links may look not pretty
        ...
        date: {
            ...
            init: function (options) {
                ...
                this.settings.datepicker =
                    $.extend({}, $.fn.editable.types.date.datepicker, directOptions, options.datepicker);
            ...
            setTextByValue: function () {
                var text = this.settings.converFormat.call(this, this.value, this.settings.format,
            ...
            setValueByText: function () {
                ...
                this.value =
                    this.settings.converFormat.call(this, text, this.settings.viewformat, this.settings.format);
            },
...
!function ($) {
    ...
    var Datepicker = function (element, options) {
        ...
        this.component = this.element.is('.date') ? this.element.find('.basket-on') : false;
    ...
    Datepicker.prototype = {
        ...
        update: function () {
            ...
            if (arguments && arguments.length && (typeof arguments[0] === 'string' || arguments[0] instanceof Date)) {
                ...
                date =
                    this.isInput ? this.element.prop('value') : this.element.data('date') || this.element.find(
                        'input').prop('value');
            }
        ...
        fill: function () {
            ...
            while (prevMonth.valueOf() < nextMonth) {
                ...
                clsName = '';
                if (prevMonth.getUTCFullYear() < year || (prevMonth.getUTCFullYear() == year && prevMonth.getUTCMonth()
                                                          < month)) {
                    clsName += ' old';
                } else if (prevMonth.getUTCFullYear() > year || (prevMonth.getUTCFullYear() == year
            ...
            for (var i = -1; i < 11; i++) {
                html +=
                    '<span class="year' + (i == -1 || i == 10 ? ' old' : '') + (currentYear == year ? ' active' : '')
        ...
        updateNavArrows: function () {
            ...
            switch (this.viewMode) {
                case 0:
                    if (this.startDate !== -Infinity && year <= this.startDate.getUTCFullYear() && month
                                                                                                   <= this.startDate.getUTCMonth()) {
                        this.picker.find('.prev').css({visibility: 'hidden'});
                    } else {
                        this.picker.find('.prev').css({visibility: 'visible'});
                    }
                    if (this.endDate !== Infinity && year >= this.endDate.getUTCFullYear() && month
                ...
                switch (target[0].nodeName.toLowerCase()) {
                    case 'th':
                        switch (target[0].className) {
                            ...
                            case 'prev':
                            case 'next':
                                var dir = DPGlobal.modes[this.viewMode].navStep * (target[0].className == 'prev' ? -1
                                    : 1);
                                switch (this.viewMode) {
                                    case 0:
        ...
        return this.each(function () {
            ...
            if (!data) {
                $this.data('datepicker',
                           (data = new Datepicker(this, $.extend({}, $.fn.datepicker.defaults, options))));
            }
